...
def main(opt, logger):
    ...
    for video_dir in tqdm(video_path, desc="Processing Videos"):  # 遍历所以的视频
        ...
        for img_name, label_data in zip(frame_names, label):
            ...
            sam_mask = np.load(os.path.join("/root/data1/brats/test_twosam_addcam_1018", img_name.split('.')[0] + '.npz'))[
                'caa_sam1_stack_pred']
        ...
        for index in np.arange(0, len(box_list)):
            if index != 0:
                ...
                for j_ in range(0, len(box)):
                    x1, y1, x2, y2 = box[j_]
                    zero_array[y1:y2 + 1, x1:x2 + 1] = 1
                if compute_iou(zero_array, original_sam_stack_mask) == 0:
                    box_list[index] = box_list[index - 1]
            else:
                box = box_list[index]
                zero_array = np.zeros_like(original_sam_stack_mask)
        ...
        for img_name, label_data in zip(frame_names, label):
            ...
            mean_huidu = process_image(img_data, img_data_mask)

            tmp_SAM_mask_caa += SAM_mask_caa[0]
            for mask_SAM in SAM_mask_caa:
                huidu = process_image(img_data, mask_SAM)
                if huidu > mean_huidu:
                    tmp_SAM_mask_caa += mask_SAM

            tmp_SAM_mask_caa[tmp_SAM_mask_caa > 0] = 1
            if process_image(img_data, tmp_SAM_mask_caa) < mean_huidu and rescnt != 0:
                tmp_SAM_mask_caa += SAM_stack[rescnt - 1]
            tmp_SAM_mask_caa[tmp_SAM_mask_caa > 0] = 1

            SAM_mask_caa = tmp_SAM_mask_caa
...
if __name__ == '__main__':
    ...
    parser.add_argument('--img_path', default="/root/data1/test/yes")
    parser.add_argument('--label_path', default="/root/data1/test/label")
    # parser.add_argument('--cam_dir', default="F:/brats/val/cam")
    parser.add_argument('--video_dir', default="/root/data1/test/valyes")
    ...
    parser.add_argument('--out_path', default="/root/data1/test/newcam")
    parser.add_argument('--video_out_path', default="/root/data1/brats/test_video_medsam_twosam_addcam_1019")
    parser.add_argument('--sam1-dir', default="/root/data1/brats/box_stack_sam1")
    parser.add_argument('--npz-dir', default="/root/data1/brats/test_twosam_addcam_1019")
