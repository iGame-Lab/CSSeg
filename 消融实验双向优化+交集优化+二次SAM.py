...
def main(opt, logger):
    ...
    for video_dir in tqdm(video_path, desc="Processing Videos"):  # 遍历所以的视频
        ...
        for img_name, label_data in zip(frame_names, label):
            ...
            sam_mask = np.load(os.path.join("/root/data1/brats/2025消融实验0.9无2次sam", img_name.split('.')[0] + '.npz'))[
                'caa_sam1_stack_pred']
        ...
        SAM_MASK[SAM_MASK > (1-parm_max)] = 1
        SAM_MASK[SAM_MASK <= (1-parm_max)] = 0
...
if __name__ == '__main__':
    ...
    parser.add_argument('--hyperparameters', default=0.9)
