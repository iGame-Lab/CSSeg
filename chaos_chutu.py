...
def main(opt, logger):
    ...
    for video_dir in tqdm(video_path, desc="Processing Videos"):  # 遍历所以的视频
        ...
        for img_name, label_data in zip(frame_names, label):
            ...
            # sam_mask = np.load(os.path.join("F:/brats/box_stack_sam1_stack/不使用二次sam的最优", img_name.split('.')[0] + '.npz'))[
            #     'caa_sam1_stack_pred']
        ...
        for index in np.arange(0, len(box_list)):
            if index != 0:
                ...
                for j_ in range(0, len(box)):
                    x1, y1, x2, y2 = box[j_]
                    zero_array[y1:y2 + 1, x1:x2 + 1] = 1
                if compute_iou(zero_array, original_cam_pro_stack_mask) == 0 and process_image(img_data_list[index],box=box_list[index])<process_image(img_data_list[index],box=box_list[index-1]):
                    box_list[index] = box_list[index - 1]
            else:
                box = box_list[index]
                zero_array = np.zeros_like(original_cam_pro_stack_mask)
        ...
        for img_name, label_data in zip(frame_names, label):
            ...
            SAM_mask_caa = bbox_prompt_demo.show(image_path=os.path.join(opt.img_path, img_name.split('.')[0] + '.npz'),
                                                 Box=box_ori_list[rescnt][0])
            for j_ in range(1, len(box_ori_list[rescnt])):
                tmp_SAM_mask = bbox_prompt_demo.show(
                    image_path=os.path.join(opt.img_path, img_name.split('.')[0] + '.npz'), Box=box_ori_list[rescnt][j_])
                SAM_mask_caa = SAM_mask_caa + tmp_SAM_mask
            SAM_mask_caa[SAM_mask_caa > 0] = 1
            ori_sam_mask.append(show_cam_on_image(tmpimage.copy(), SAM_mask_caa, returncam=True,box=box_ori_list[rescnt]))
        ...
        for img_name, label_data in zip(frame_names, label):
            ...
            tmpimage = np.array(temp)
            tmpimage = tmpimage.transpose(1, 2, 0)
            dice_sam = dice_coeff(SAM_MASK_LIST[rescnt], label_data)
            dice_sam_caa += dice_sam
            sam2_dice.append(dice_sam)
            SAM_list.append(show_cam_on_image(tmpimage.copy(), SAM_MASK_LIST[rescnt], returncam=True,box=box_list[rescnt]))
        ...
        if not os.path.exists(video_output_path):
            os.makedirs(video_output_path)
        for t in np.arange(0,len(cam_list)):
            concatenate_images_and_save(img_list[t],cam_list[t],ori_sam_mask[t],SAM_list[t],label_res_list[t],output_path=os.path.join(video_output_path, f"{t}.png"))
        # # 保存原始cam为视频。
        ...
        # save_images_as_video(ori_sam_mask,os.path.join(video_output_path, "oriSAM.mp4"))
        # dice_list = np.array([cam_dice, cam_box_dice, sam2_dice,label_dice ])
        # merge_videos([
        #     os.path.join(video_output_path, "原始cam.mp4"),
        #     os.path.join(video_output_path, "oriSAM.mp4"),
        #     os.path.join(video_output_path, "SAM.mp4"),
        #     os.path.join(video_output_path, "label.mp4")
        # ], output_path=os.path.join(video_output_path, "combine.mp4"), titles=["原始cam", "medsam", "our", "gt"],
        #     dice=dice_list)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--img_path',default="F:/brats/Train_Sets/val/val/yes")
    parser.add_argument('--label_path',default="F:/brats/Train_Sets/val/val/label")
    # parser.add_argument('--cam_dir', default="F:/brats/val/cam")
    parser.add_argument('--video_dir', default="F:/brats/Train_Sets/val/val/valyes")
    ...
    parser.add_argument('--out_path',default="F:/brats/Train_Sets/val/val/newcam")
    parser.add_argument('--video_out_path', default="F:/brats/Train_Sets/val/val/video_twosam_addcam_10223")
    parser.add_argument('--sam1-dir', default="/root/data1/brats/box_stack_sam1")
    parser.add_argument('--npz-dir', default="F:/brats/Train_Sets/val/val/twosam_addcam_10223")
