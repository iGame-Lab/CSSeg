...
def main(opt,logger):
    ...
    for i,img_name in enumerate(os.listdir(opt.img_path),1):
        ...
        if dice_coeff(CRF_label,label_data)<0.2:
            ...
            logger.info("图片:{},dice_crf{},dice_caa_cam:{}".format(img_name,dice_coeff(CRF_label,label_data),dice_coeff(caa_grad_cam,label_data)))

        if i%30==0:
            logger.info("\ndice_crf_mean:{},dice_caa_mean:{}".format(dice_CAM_CRF / i,dice_caa/i))
    logger.info("\ndice_sum:{}".format(dice_CAM_CRF / len(os.listdir(opt.img_path))))
