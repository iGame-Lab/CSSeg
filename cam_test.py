...
def main(opt,logger):
    ...
    for i,img_name in enumerate(os.listdir(opt.img_path),1):
        ...
        # cam_refined_highres=1-cam_refined_highres
        # cam_refined_highres=preprossess(img_data,cam_refined_highres)
        show_cam_on_image(tmpimage, cam_refined_highres, use_rgb=True,
                          name="caa_grad_cam")
        show_cam_on_image(tmpimage, grad_cam.numpy(), use_rgb=True,
                          name="grad_cam")

def dice_computer_cam(opt,logger):
    for threshold in np.arange(0.85,1,0.05).round(2):
        dice_grad_cam=0
        dice_caa_grad_cam=0
