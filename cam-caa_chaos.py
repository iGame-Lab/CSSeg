...
def main(opt,logger):
    ...
    weights_path = "F:/Brats-class/vision_transformer/chaoweights/model-27.pth"
...
def dice_computer_cam(opt,logger,max_dice):
    for threshold in np.arange(0.7,0.9,0.05).round(2):
        dice_grad_cam=0
        dice_caa_grad_cam=0
        ...
        for i,img_name in enumerate(os.listdir(opt.img_path),1):
            ...
            label_data = np.load(os.path.join(opt.label_path, img_name))["arr_0"]
            label_data[label_data > 0] = 1
            label_data = cv2.resize(np.array(label_data), (256, 256), interpolation=cv2.INTER_NEAREST)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--img_path',default="F:/brats/Train_Sets/train/train/yes")
    parser.add_argument('--label_path',default="F:/brats/Train_Sets/train/train/label")
    parser.add_argument('--not-save', default=False, action='store_true',
                          help='If yes, only output log to terminal.')
    parser.add_argument('--out_path',default="F:/brats/Train_Sets/train/train/newcam")
