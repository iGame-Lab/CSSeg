...
def main(opt, logger):
    ...
    for video_dir in tqdm(video_path, desc="Processing Videos"):  # 遍历所以的视频
        ...
        for img_name, label_data in zip(frame_names, label):
            ...
            caa_grad_cam[caa_grad_cam < parm_max * caa_grad_cam.max()] = 0
            caa_grad_cam[caa_grad_cam >= parm_max * caa_grad_cam.max()] = 1
        ...
        cam_pro_stack_mask[cam_pro_stack_mask >= (1-parm_max)] = 1
        cam_pro_stack_mask[cam_pro_stack_mask < (1-parm_max)] = 0
        ...
        logging.info(f"{video_name}:dice_cam:{round(np.array(cam_box_dice).sum() / cam_box_dice.__len__(), 4)}"
                     f"dice_sam:{round(np.array(sam2_dice).sum() / sam2_dice.__len__(), 4)},dice_cam_sum: {round(dice_caa / cnt, 4)} dice_sam_sum: {round(dice_sam_caa / cnt, 4)},cnt:{cnt}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--video_out_path', default="/root/data1/brats/2025消融实验视频")
    parser.add_argument('--sam1-dir', default="/root/data1/brats/box_stack_sam1")
    parser.add_argument('--npz-dir', default="/root/data1/brats/2025消融实验")
    parser.add_argument('--hyperparameters',default=0.7)
