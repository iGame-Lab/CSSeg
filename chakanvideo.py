...
def main(opt, logger):
    ...
    for video_dir in tqdm(video_path, desc="Processing Videos"):  # 遍历所以的视频
        ...
        for img_name, label_data in zip(frame_names, label):
            ...
            sam_mask = np.load(os.path.join("F:/brats/twosam_addcam_1020",img_name.split('.')[0] + '.npz'))['caa_sam1_stack_pred']
        ...
        for index in np.arange(len(box_list) - 1, -1, -1):  # 从最后一个元素开始反向遍历
            ...
            if index != len(box_list) - 1:  # 确保不是最后一个元素
                if not np.any(np.logical_and(cam_ori == 1, sam_pro_mask == 1)):
                    box_list[index] = box_list[index + 1]  # 将当前的 box 替换为下一个 box 的值
            else:
                if not np.any(np.logical_and(cam_ori == 1, sam_pro_mask == 1)):
                    box_list[index] = box_list[index - 1]
        ...
        for img_name, label_data in zip(frame_names, label):
            ...
            label_dice.append(1)
            SAM_mask_caa = np.load(os.path.join(opt.sam1_dir,img_name.split('.')[0]+'.npz'))['caa_sam1_stack_pred']
...
if __name__ == '__main__':
    ...
    parser.add_argument('--video_out_path', default="F:/brats/video_1020_best")
    parser.add_argument('--sam1-dir', default="F:/brats/twosam_addcam_1020")
