...
def AMS(cam_pro_stack_mask, cam_pro_mask):
    return [np.logical_and(arr, cam_pro_stack_mask) for arr in cam_pro_mask]
...
def main(opt, logger):
    ...
    for video_dir in tqdm(video_path, desc="Processing Videos"):  # 遍历所以的视频
        ...
        cam_pro_mask = AMS(cam_pro_stack_mask,cam_pro_mask)

        similarity_AMS = compute_similarity(cam_pro_mask)
