...
def main(opt, logger):
    ...
    for video_dir in tqdm(video_path, desc="Processing Videos"):  # 遍历所以的视频
        ...
        for img_name, label_data in zip(frame_names, label):
            ...
            sam_mask = np.load(os.path.join("/root/data1/brats/twosam_addcam_1018", img_name.split('.')[0] + '.npz'))[
                'caa_sam1_stack_pred']
        ...
        for img_name, label_data in zip(frame_names, label):
            ...
            json_data = {
                ...
                "bounding_boxes": box_list[rescnt].tolist()  # 将 numpy 数组转换为列表，以便 JSON 序列化
            }
            output_json_data.append(json_data)


        logging.info(f"{video_name}:dice_cam:{round(np.array(cam_box_dice).sum() / cam_box_dice.__len__(), 4)}"
                     f"dice_cam_sum: {round(dice_caa / cnt, 4)} ")
        with open("F:/new_cam-MEDSAM/output.json", "w") as f:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--video_out_path', default="/root/data1/brats/video_twosam_addcam_1031")
    parser.add_argument('--sam1-dir', default="/root/data1/brats/box_stack_sam1")
    parser.add_argument('--npz-dir', default="/root/data1/brats/twosam_addcam_1031")
